...
#' ovlTest(X1, X2)
#'
ovlTest <- function(X1, X2){
  n1 <- nrow(X1)
  n2 <- nrow(X2)
  S1 <- cov(X1)
  S2 <- cov(X2)

  TS <- sum((colMeans(X1) - colMeans(X2))^2) -
  ...
  c1 <- (sum(A1^2) * (n1 - 1)^2 - sum((rowSums(A1))^2) * (2 * n1 - 1) +
           (sum(A1))^2) / (n1 * (n1 - 1) * (n1 - 2)^2)
  c2 <- (sum(A2^2) * (n2 - 1)^2 - sum((rowSums(A2))^2) * (2 * n2 - 1) +
           (sum(A2))^2) / (n2 * (n2 - 1) * (n2 - 2)^2)
  sigsq <- 2* c1 / n1 / (n1 - 1) + 2 * c2 / n2 / (n2 - 1) + 4 * c12 / n1 / n2
